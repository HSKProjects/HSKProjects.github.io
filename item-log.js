(function () {
  ...
  function render() {
    ...
    Object.keys(groupedLogs).forEach((date) => {
      ...
      title.innerHTML = highlightMatch(formatDate(date), query);
      count.textContent = `${rows.length} ${rows.length === 1 ? "entry" : "entries"}`;

      rows.forEach((log) => {
        const tr = document.createElement("tr");
        if (query && logMatchesQuery(log, query)) {
          tr.classList.add("matched-row");
        }
        tr.innerHTML = `
          <td>${highlightMatch(formatTime(log.createdAt), query)}</td>
          <td>${highlightMatch(log.person, query)}</td>
          <td>${highlightMatch(log.item, query)}</td>
          <td>${highlightMatch(log.notes || "", query)}</td>
